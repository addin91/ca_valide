        ...
        foreach ($b['ues'] as $uk => $ue) {
            ...
            foreach ($kids as $t_info) {
                if (isset($posted[$t_info['id']])) {
                    $p = $posted[$t_info['id']];
                    $cc = (isset($p['cc']) && $p['cc'] !== '') ? $p['cc'] : null;
                    $pa = (isset($p['partiel']) && $p['partiel'] !== '') ? $p['partiel'] : null;
                    $ex = (isset($p['examen']) && $p['examen'] !== '') ? $p['examen'] : null;
...
<header>
    ...
    <nav>
        ...
        <?php if ($user_id): ?>
            <a href="logout.php" style="margin-left: 1rem; color: var(--danger-color); font-weight: bold;">Déconnexion</a>
        <?php endif; ?>
    </nav>
</header>
<div class="container">
    ...
    <div class="blocs-grid">
        <?php foreach($blocs as $index=>$b): ?>
            <div class="bloc-card">
                <div class="bloc-header" onclick="toggleBloc('bloc-<?php echo $index; ?>')">
                    <div>
                        <div class="bloc-name"><?= $b['nom'] != "Bloc Transverse" ? "Bloc " . $index+1 . " <span>". htmlspecialchars($b['nom']) ."</span>" : htmlspecialchars($b['nom']) ?></div>
                ...
                <!-- UEs WRAPPED in Collapsible Div -->
                <div id="bloc-<?php echo $index; ?>" class="bloc-content" style="display:none;">
                    <?php foreach($b['ues'] as $ue): ?>
                        <?php 
                        $kids=!empty($ue['children'])?$ue['children']:[$ue];
                        
                        // FILTER: Only show kids with grades if any exist
                        $kids_with_grades = [];
                        foreach($kids as $k) {
                        ...
                        if(!empty($ue['children'])): ?>
                             <div style="font-weight:bold; font-size:0.9rem; margin-top:0.5rem;"><?php echo htmlspecialchars($ue['nom']); ?></div>
                        <?php endif; ?>
                        ...
                        <?php foreach($kids as $k): ?>
                            <div class="ue-row">
                                <div class="ue-name">
                                    ...
                                    <span style="font-size:0.8rem; color:#888;">(<?php echo $k['ects']; ?> ECTS)</span>
                                </div>
                                <div class="ue-grade <?php echo ($k['moyenne']!==null && $k['moyenne']>=10)?'text-green':'text-red'; ?>">
                            ...
                            <?php if(isset($sim_res[$k['id']])): ?>
                                <div class="sim-row">
                                    <?php 
                                    $p=[]; foreach($sim_res[$k['id']] as $n=>$v) $p[]="$n > ".number_format($v,2);
                                    echo implode(", ", $p);
                                    ?>
                                </div>
                            <?php endif; ?>
